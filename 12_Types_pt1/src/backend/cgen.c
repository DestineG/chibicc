...
int cgloadglob(int id) {
    int reg = alloc_reg();
    if(Gsym[id].type == P_CHAR) {
        fprintf(Outfile, "\tmovzbq %s(\%%rip), %s\n", Gsym[id].name, reglist[reg]);
    }
    else if (Gsym[id].type == P_INT) {
        fprintf(Outfile, "\tmovq %s(\%%rip), %s\n", Gsym[id].name, reglist[reg]);
    }
    else {
        fatald("Unknown type in cgloadglob()", Gsym[id].type);
    }
    return reg;
}
...
int cgstorglob(int r, int id) {
  if(Gsym[id].type == P_CHAR) {
    ...
    fatald("Unknown type in cgstorglob()", Gsym[id].type);
  }
  return (r);
}
...
void cgglobsym(int id) {
  if(Gsym[id].type == P_CHAR) {
    ...
    fatald("Unknown type in cgglobsym()", Gsym[id].type);
  }
}
